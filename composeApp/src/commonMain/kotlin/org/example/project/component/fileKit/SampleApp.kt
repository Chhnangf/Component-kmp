...
fun SampleApp(screenModel: PhotoScreenModel) {
    ...
    var file by remember { mutableStateOf((emptyList<ByteArray>())) }
...
//                )
        ...
        Button(onClick = {
            ...
            println("addFileImage -> $fileObject")
            screenModel.addFileImage(fileObject)
        }) {
            Text("发布")
        }
        }
...
fun DraggableDialog(
    ...
    val closeThreshold = 400f // 关闭预览的需求偏移量
...
//                                    // 切换到上一张图片
                            ...
                            backgroundAlpha =
                                backgroundAlpha.coerceIn(0f, 1f) // 确保 Alpha 在 0zai  到 1 之间

                            // 更新 totalDragDistance，只累加拖动距离的绝对值
                            totalDragDistanceX += -dragAmount.x
                            totalDragDistanceY += kotlin.math.abs(dragAmount.y)
                            println("totalDragDistanceX -> totalDragDistanceX:$totalDragDistanceX offsetX:$offsetX")
